...
import type { ISignupResponse } from "../entities/auth.entities";
import type { ILoginResponse } from "../../auth/auth.entities";
...
export class AuthenticationService {
     ...
     */
    signupWithGmail = async (idToken: string): Promise<IResponse<ISignupResponse>> => {
     ...
     * @returns Promise<Respone>
     */
    loginWithGmail = async (idToken: string): Promise<IResponse<ILoginResponse>> => {
        if (!idToken || typeof idToken !== 'string') {
            throw new BadRequestException("Invalid Id Token!");
        }
     ...
     * @returns Promise<Respone>
     */
    signWithGmail = async (idToken: string): Promise<IResponse<ISignupResponse | ILoginResponse>> => {
        if (!idToken || typeof idToken !== 'string') {
            throw new BadRequestException("Invalid Id Token");
        }
     ...
     */
    signup = async (data: ISignup): Promise<IResponse<ISignupResponse>> => {
     ...
     * @returns Promise<Response>
     */
    login = async ({ email, password }: ILogin): Promise<IResponse<ILoginResponse>> => {
        const user: UserDoc | UserDocLean | null = await this.userModel.findUser({
            filter: { email }
        });
        if (!user) {
        ...
        if (user.twoSV) {
            ...
            return {
                message: "Check Your Email To Login With OTP",
                statusCode: 200
            };
        }
    ...
    twoSVLoginConfirmation = async ({ email, otp }: IEmailConfirmation): Promise<IResponse<ILoginResponse>> => {
        const user: UserDoc | UserDocLean | null = await this.userModel.findUser({
            filter: { email }
        });
        if (!user) {
     ...
     * @returns Promise<Respone>
     */
    confirmEmail = async ({ email, otp }: IEmailConfirmation): Promise<IResponse> => {
        const user: UserDoc | UserDocLean | null = await this.userModel.findUser({
            filter: { email: email }
        });
        if (!user) {
     ...
     * @returns Promise<Respone>
     */
    resendConfirmEmail = async (email: string): Promise<IResponse> => {
        const user: UserDoc | UserDocLean | null = await this.userModel.findUser({
            filter: { email }
        });
        if (!user) {
        ...
        return {
            message: `OTP Code's Been Sent Successfully to ${email}`,
            statusCode: 200
        };
    };
};